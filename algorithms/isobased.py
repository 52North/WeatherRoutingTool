...
class IsoBased(RoutingAlg):
    ...
    def init_fig(self, wt, showDepth = True):
        ...
        if(self.showDepth):
            self.depth = wt.ds['depth'].where(wt.ds.depth < 0, drop=True)
    ...
    def generate_basemap(self):
        ...
        if(self.showDepth):
            level_diff = 10
            cp = self.depth.plot.contourf(ax=self.ax, levels=np.arange(-100, 0, level_diff),
