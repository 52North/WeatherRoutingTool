...
class Population(Sampling):
    ...
    def check_validity(self, routes):
        """
        Check whether the waypoints of the routes violate constraints.
        Raise a warning for each single route that violates constraints.
        Raise an error, if more than 50% of all routes violate constraints.
...
class GridBasedPopulation(GridMixin, Population):
    ...
    def __init__(self, default_route, grid, constraints_list, pop_size):
        ...
        rf = fs[np.where(utils.get_constraints_array(fs, constraints_list))]
        for lat, lon in rf:
    ...
    def generate(self, problem, n_samples, **kw):
        ...
        for i in range(n_samples):
            ...
            X[i, 0] = np.array([self.src, *route[1:-1], self.dst])
...
class FromGeojsonPopulation(Population):
    ...
    NOTE: Routes are expected to be named in the following format:
    `route_{1..N}.json`
    ...
    def generate(self, problem, n_samples, **kw):
        ...
        for i in range(n_samples):
            ...
            if not os.path.exists(path):
                raise ValueError(
                    "The number of available routes for the initial population does not match the "
                    "population size."
                )
            else:
...
class IsoFuelPopulation(Population):
    ...
    Produces initial routes using the IsoFuel algorithm's ISOCHRONE_NUMBER_OF_STEPS configuration.
    If the number of generated routes is lesser than the expected n_samples number of individuals,
    the last produced route is repeated until the required number of individuals are met
    """
    ...
    def __init__(self, config: Config, boat: Boat, default_route, constraints_list, pop_size):
        ...
        self.patcher = PatchFactory.get_patcher(
            config=config,
            patch_type="isofuel_multiple_routes",
    ...
    def generate(self, problem, n_samples, **kw):
        ...
        for i, rt in enumerate(routes):
            route_with_speed = []
            for pt in [self.src, *rt[1:-1], self.dst]:
                route_with_speed.append([pt[0], pt[1], self.speed])
            X[i, 0] = np.array(route_with_speed)
...
class GcrSliderPopulation(Population):
    ...
    def generate(self, problem, n_samples, **kw):
        """
        Create an initial population with good variety of routes by calculating routes using the
        GCR Slider Algorithm with different waypoints.

        Each route is created with only one waypoint. Waypoints are created in two steps:
        ...
        2) For each point move the point in orthogonal direction (clockwise and counter-clockwise).
           Increase the distance used to move the point incrementally.
        """
        ...
        while len(routes) < n_samples:
            ...
            if not self.algo.is_land(lat, lon):
                ...
                if route is not None:
                    ...
                    logger.info(
                        f"Could not find a new route (dist_fraction={dist_fraction}, "
    ...
    def create_route(self, lat: float = None, lon: float = None):
        ...
        try:
            ...
            route = [
                [route.lats_per_step[i], route.lons_per_step[i]]
                for i in range(0, len(route.lats_per_step))
            ]
