...
def execute_routing(config):
    ...
    out_dir = Path(routepath)
    out_dir.mkdir(parents=True, exist_ok=True)
    min_fuel_route.return_route_to_API(str(out_dir / f"{min_fuel_route.route_type}.json"))
    ...
    if config.ROUTE_POSTPROCESSING:
        ...
        min_fuel_route_postprocessed.return_route_to_API(str(out_dir / f"{min_fuel_route_postprocessed.route_type}_postprocessed.json"))
    # prof.disable()
